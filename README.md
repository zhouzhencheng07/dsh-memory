...
| **检索（`recall` + `keywords`）** | 标题感知块级检索（任意标题切块、带面包屑）、**单字段位置加权关键词**（2026-08-25）：一个 `keywords` 参数最多 5 个空格分隔的词——**前 3 个高权重**（每个命中 ×3）、后 2 个低权重（每个命中 ×1），逐词部分分、无硬 AND 门禁；**覆盖率分数线 `MIN_COVERAGE=0.3`**（2026-08-29）：块须占本次查询实际可用证据的至少 30%，词多只中一个会被掐、错词/泛词不抬高门槛；**IDF 词权重**（2026-08-29）：按全库文档频率归一，**稀有词保持原权重、全库皆有的泛词权重趋零**（泛词独力撑不过 `MIN_SCORE=0.5`，专治高频词拽出无关块），某词全库无命中或人人命中都会在提示里说明；**ASCII 词边界**（2026-08-29）：纯字母词按边界匹配（`log` 不再命中 `catalog`），带数字/点号的版本串与 CJK 保持子串语义；去格式符宽容匹配、命中数按块长归一、**按日衰减**（30 天半衰、下限 0.4）；snippet 返回整个块（≤1000 字符）。**固化/修正指引按结果组成分支附加在输出末尾**：命中长期块 → 以它为准、过时就地修正、重叠主题合并；日记命中带**复现证据**（日期≠今天 / 跨工作区）→ 催晋升 `topics/<topic>.md`，全为今日本工作区命中 → 不催（晋升门槛，A）；空结果只报关键词健康、不给固化指引。末尾还会附加**单行维护提示**（单话题 ≥16KB 时点名，见"长期层三道约束"C） |
| **两层记忆库** | 日记层 `YYYY-MM-DD/`（流水，**硬窗口** `dailyWindowDays` 默认 45 天——agent 迭代快，过期笔记不再参与检索但保留在磁盘；0=不限）+ 长期层 `topics/<topic>.md`（**自由主题分文件**，一个主题一个文件，永不衰减、不受窗口限制）。分工判据：**换个项目还成立的经验进长期层**（环境/工具教训、协作偏好、通用模式）；事件流水进日记；**常驻约定进 AGENTS.md（保持薄），不进记忆**；项目内偶尔需要的知识 → 日记暂存、复现才晋升；**能从代码/文档/工作区自身说明推导的不记**（skip rule）。**长期层只在复用时固化**——晋升门槛见下一行（A），不在捕获时预判。**旧日记只读**（`remember` 带 `date` 一律拒绝），无计数器、零状态文件 |
| **长期层三道约束（2026-09-04）** | 约束只落在**有证据的时刻**，不做计数阈值：**晋升门槛（A）**——新长期素材先写今日日记，**再次被需要**（命中行日期≠今天 / 跨工作区，recall 提示在该证据出现时才催晋升）才进 `topics/`；可推导的不记，用户偏好/环境常量这类恒真窄类可直入；**写入期查重（B）**——新建 topic 文件时先以笔记标题词检索长期语料，强重叠（`DEDUP_SCORE=2`）即拒绝并给既有地址，`recall` 后往里并（编辑路径与日记不查重，弱重叠放行）；**单信号维护（C）**——单话题文件 ≥16KB（`TOPIC_NOTICE_BYTES`）时 recall 末尾附**一行**低压力提示、点名该文件（只提示不动作）。话题数/总量阈值被有意否决：话题数是工作量的函数（忙≠乱），块级检索不受话题数影响——计数类阈值没有证据时刻 |
| **话题不列清单（2026-09-01）** | 工具输出**任何地方都不列全部长期话题**：清单会无限膨胀成噪音，并诱导 agent 按文件名浏览而非检索。想知道某个话题存在，唯一途径是它在某次 `recall` 结果里浮现；命中行给出 `topics/<name>`，再用 `topic` 参数读/改。**唯一例外**：C 的维护提示在触发时点名超限的那一个文件（触发域限定，不是清单） |
...
- **两层语义**：事件流水写日记；长期主题文件只在**复现时晋升**——recall 的晋升提示仅在命中带复现证据（日期≠今天 / 跨工作区）时出现（A），新建 topic 前对长期语料查重、强重叠拒绝并指去既有文件（B）；不在捕获时预判
