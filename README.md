...
| **memory_search 工具** | 标题感知块级检索（任意标题切块、带面包屑）、**两组关键词加分**（2026-08-24）：`primary` ≤2 个高权重（每个命中 ×3）+ `secondary` ≤3 个低权重（每个命中 ×1），逐词部分分、无硬 AND 门禁——块里缺某个词不再整条消失；去格式符宽容匹配（反引号/引号/加粗记号不切断命中）、命中数按块长归一、**按日衰减**（30 天半衰、下限 0.4——旧笔记降权但不会消失）；snippet 返回整个块（≤1000 字符，通常无需再开文件） |
